...
@onready var jumpsound=$soundmanger/jumpsound
@onready var runsound=$soundmanger/runsound
var MAX = 5 #最大胶卷数量
var in_air = 0 #落地判断变量
var h_yorn = int(true) #水平方向是否可操作
var v_yorn = int(true) #竖直方向是否可操作
var Number_of_films = 2#人物胶片数量
...
func _ready():
	...
	$"../车厢".connect("feedback4",Callable(self,'change_flag'))
...
func _physics_process(delta): #人物方法
	...
	if Input.is_action_just_pressed('space') and v_yorn and is_on_floor(): #跳跃函数
		velocity.y = v_y 
