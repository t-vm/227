...
func f16_(): #对话函数16（第二关，捡起第一个胶卷的强制对话。 ）
	Dialogbox.show_dialogbox([{name='',text='用户指南'},{name='',text='优秀的摄影作品引人驻足欣赏，伟大的摄影作品改变整个世界！'},{name='',text='感谢您选择x-0e型相机与配套胶卷。'},{name='',text='本产品致力于让每一位用户以最简单的学习成本发挥独一无二的创意，拍摄伟大的摄影作品。'},{name='',text='*按键说明*'},{name='',text='按E并使用滚轮选择拍摄模式。使用回车选中以进行具体参数调节，或再按一次E退出。'},{name='',text='在参数调节界面，您需要使用滚轮选择参数，回车选中后按q使用取景框，或按esc返回上级。'},{name='',text='在取景框界面，您可以用光标定位取景框位置。随后，点击左键确认并锁定后，按S拍照，查看预览效果。随后回车确认效果，或是点击右键取消本次拍照。'},{name='',text='当您选择好一次具体模式和参数后，以后便可以直接按q进行拍照操作。您可以随时对模式和参数进行修改。'},{name='',text='令人愉悦的契合声让我明白相机大概已经被正确填入胶卷，可以使用了。'},{name='我：',text='果然可以找到。'},{name='我：',text='但这显然不是一个可以让我随意发挥的数量。'}])
